...
def generer_labels(chemin_atomic_boxes, cases_json, dossier_cases):
    ...
    if not atomic_cases or not cases:
        log("Aucune case ou atomic_case disponible.", "WARN")
        return None

    ax_vals = [a["cx"] for a in atomic_cases]
    ay_vals = [a["cy"] for a in atomic_cases]
    ax_min, ax_max = min(ax_vals), max(ax_vals)
    ay_min, ay_max = min(ay_vals), max(ay_vals)
    ax_range = ax_max - ax_min or 1.0
    ay_range = ay_max - ay_min or 1.0
    for a in atomic_cases:
        a["nx"] = (a["cx"] - ax_min) / ax_range
        a["ny"] = (a["cy"] - ay_min) / ay_range

    pages = defaultdict(list)
    for case in cases:
        pages[case.get("page", 1)].append(case)

    labels = {}

    for page_num, cases_page in pages.items():
        case_coords = []
        for case in cases_page:
            ...
            if tl is None:
                continue
            cx = (tl[0] + tr[0]) / 2 if tr else tl[0]
            cy = (tl[1] + bl[1]) / 2 if bl else tl[1]
            case_coords.append({
                "id":   case["id"],
                "page": page_num,
                "cx":   cx,
        ...
        cx_vals = [c["cx"] for c in case_coords]
        cy_vals = [c["cy"] for c in case_coords]
        cx_min, cx_max = min(cx_vals), max(cx_vals)
        cy_min, cy_max = min(cy_vals), max(cy_vals)
        cx_range = cx_max - cx_min or 1.0
        cy_range = cy_max - cy_min or 1.0
        for c in case_coords:
            c["nx"] = (c["cx"] - cx_min) / cx_range
            c["ny"] = (c["cy"] - cy_min) / cy_range

        used_atomic = set()
        case_coords_sorted  = sorted(case_coords,  key=lambda c: (round(c["ny"], 1), c["nx"]))
        atomic_cases_sorted = sorted(atomic_cases, key=lambda a: (round(a["ny"], 1), a["nx"]))

        for c in case_coords_sorted:
            best_dist  = float("inf")
            best_idx   = None
            best_label = None
            for i, a in enumerate(atomic_cases_sorted):
                if i in used_atomic:
                    continue
                dist = ((a["nx"] - c["nx"])**2 + (a["ny"] - c["ny"])**2) ** 0.5
                if dist < best_dist:
                    best_dist  = dist
                    best_idx   = i
                    best_label = a["label"]
            if best_idx is not None and best_dist < 0.99:
                key = f"page_{page_num:03d}_case_{c['id']}"
                labels[key] = best_label
                used_atomic.add(best_idx)
    ...
    if len(labels) == 0:
        log("ATTENTION : 0 labels generes — verifiez atomic-boxes.json et cases.json", "WARN")
