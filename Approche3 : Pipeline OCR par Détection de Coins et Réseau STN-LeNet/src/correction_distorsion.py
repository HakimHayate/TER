...
def extraire_pages_rapide(chemin_pdf: str, dossier_sortie: str) -> list[str]:
    """
    Extrait rapidement les pages d'un PDF avec pdfimages
    et les renomme au format page_001.png, page_002.png, ...
    Retourne la liste des chemins des images extraites.
    """
    os.makedirs(dossier_sortie, exist_ok=True)

    # Nettoyer d'anciens fichiers temporaires éventuels
    for f in glob.glob(os.path.join(dossier_sortie, "img-*")):
        try:
            os.remove(f)
        except OSError:
            pass

    prefix = os.path.join(dossier_sortie, "img")

    try:
        subprocess.run(["pdfimages", "-png", chemin_pdf, prefix], check=True)

        fichiers = sorted(
            f for f in os.listdir(dossier_sortie)
            if f.startswith("img-") and f.lower().endswith(".png")
        )

        chemins_pages = []
        for i, nom in enumerate(fichiers, start=1):
            ancien = os.path.join(dossier_sortie, nom)
            nouveau = os.path.join(dossier_sortie, f"page_{i:03d}.png")

            # Si un ancien page_XXX.png existe déjà, on l'écrase proprement
            if os.path.exists(nouveau):
                os.remove(nouveau)

            os.rename(ancien, nouveau)
            chemins_pages.append(nouveau)

        return chemins_pages

    except FileNotFoundError:
        raise RuntimeError(
            "pdfimages introuvable. Installe poppler-utils : sudo apt install poppler-utils"
        )
    except subprocess.CalledProcessError as e:
        raise RuntimeError(f"Extraction rapide échouée avec pdfimages : {e}")
# ─────────────────────────────────────────────────────────────────────────────
# CONSTANTES
# ─────────────────────────────────────────────────────────────────────────────
...
def executer_correction(chemin_pdf: str, chemin_json: str,
    ...
    # ── Extraction rapide des pages du PDF ──
    pages_extraites = extraire_pages_rapide(chemin_pdf, dossier_sortie)
    print(f"  {len(pages_extraites)} page(s) détectée(s)\n")

    resultats = []
    for i, chemin_image in enumerate(pages_extraites, start=1):
        page_bgr = cv2.imread(chemin_image, cv2.IMREAD_COLOR)

        if page_bgr is None:
            print(f"  ✗ Impossible de lire : {chemin_image}")
            resultats.append({"page": i, "fichier": None, "statut": "echec_lecture"})
            continue
