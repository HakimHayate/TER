...
EMPTY_DENSITY_THRESHOLD = 0.025   # 1 % → case considérée vide
...
def segmenter_chiffre(binaire: np.ndarray) -> np.ndarray:
    ...
    c = max(valid_contours, key=lambda x: cv2.boundingRect(x)[2]*cv2.boundingRect(x)[3])
    x, y, w, h = cv2.boundingRect(c)
...
def traiter_case(page_gris: np.ndarray, case: dict,
    ...
    """
    case_id = case.get("id", "?")
    page_num = case.get("page", 1)
    unique_id = f"page_{page_num:03d}_case_{case_id}"

    res = {"id": unique_id, "statut": "non_lisible",
        "coins_utilises": 0, "fichier": None, "erreur": None}
    pfx = os.path.join(DEBUG_DIR, unique_id)
    try:
        ...
        # ── 3. Vide ? ──
        if est_vide(bin_img):
            chemin = _save(np.zeros((TARGET_SIZE, TARGET_SIZE), dtype=np.uint8), case_id, page_num)
        ...
        nb_pixels = np.count_nonzero(chiffre)
        h, w = chiffre.shape
        density = nb_pixels / (h * w) if h * w > 0 else 0

        if nb_pixels < 30 or density < 0.015:
            res["statut"] = "bruit"
            return res
        # =================================================

        # ================= FILTRE FORME =================
        ys, xs = np.where(chiffre > 0)
        ...
        chemin = _save(finale, case_id, page_num)
...
def _save(img: np.ndarray, case_id: str, page_num: int) -> str:
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    chemin = os.path.join(OUTPUT_DIR, f"page_{page_num:03d}_case_{case_id}.png")
...
def executer_pipeline(chemin_json: str, chemin_pdf: str = None,
    ...
    # Calculer une homographie par page
    homographies = {}
    for page_num, page_img in pages.items():
        try:
            homographies[page_num] = calculer_homographie_mm_vers_px(page_img)
        except Exception as e:
            print(f"[WARN] Homographie impossible pour page {page_num} : {e}")
            homographies[page_num] = None

    # Conversion des coins JSON (mm) -> pixels avec l'homographie de la bonne page
    for case in cases:
        page_num = case.get("page", 1)
        H_mm_to_px = homographies.get(page_num)
    ...
    for s in ["ok", "vide", "approximatif", "non_lisible", "bruit", "forme_invalide"]
}

    print(f"\n{'─'*60}")
    print(f"  ✓ ok={stats['ok']}  ≈ approx={stats['approximatif']}"
      f"  ○ vides={stats['vide']}  ✗ non_lisibles={stats['non_lisible']}"
      f"  ~ bruit={stats['bruit']}  △ forme_inv={stats['forme_invalide']}")
