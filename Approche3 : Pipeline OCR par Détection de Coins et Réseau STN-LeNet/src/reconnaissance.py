...
def reconnaitre_dossier(dossier):
    ...
    for fichier in sorted(os.listdir(dossier)):
        ...
        # Extraire l'id de case depuis le nom du fichier PNG
        # "page_001_case_c12.png" → "page_001_c12"
        # "case_c12.png"          → "c12"
        nom_sans_ext = os.path.splitext(fichier)[0]
        import re as _re
        m = _re.match(r'(page_[0-9]+)_case_(c[0-9]+)$', nom_sans_ext)
        if m:
            case_id = f'{m.group(1)}_case_{m.group(2)}'  # "page_001_c12"
        else:
            m2 = _re.search(r'(c[0-9]+)$', nom_sans_ext)
            case_id = m2.group(1) if m2 else nom_sans_ext
    ...
    for r in resultats_json:
        # r["id"] format: "page_001_case_c1"
        rid = r["id"]
        # short key for predictions dict: "page_001_c1"
        case_id_short = rid.replace("_case_", "_", 1) if "_case_" in rid else rid.replace("case_", "", 1)
        # full key for labels dict: "page_001_case_c1" (same as r["id"])
        case_id_full  = rid

        pred = predictions.get(case_id_short) or predictions.get(rid)
        conf = confidences.get(case_id_short) or confidences.get(rid)
        if pred is not None:
            r["chiffre_predit"] = pred
            r["confiance"]      = round(conf, 1)
        # labels.json uses full key: "page_001_case_c1"
        if case_id_full in labels:
            r["label_reel"] = labels[case_id_full]

    # ── Calculer accuracy globale et la stocker dans le premier élément ──
    # predictions keys: "page_001_c1"
    # labels keys:      "page_001_case_c1"
    # bridge: expand predictions keys to full format for matching
    predictions_full = {}
    for k, v in predictions.items():
        import re as _re
        m = _re.match(r'(page_[0-9]+)_(c[0-9]+)$', k)
        if m:
            full_key = f"{m.group(1)}_case_{m.group(2)}"
        else:
            full_key = k
        predictions_full[full_key] = v

    ids_communs = [k for k in predictions_full if k in labels]
    accuracy = None
    if ids_communs:
        correct = sum(1 for k in ids_communs if predictions_full[k] == labels[k])
    ...
    if labels:
        ...
        if ids_communs:
            ...
            y_pred = [predictions_full[k] for k in ids_communs]
